document.addEventListener('DOMContentLoaded', () => {
  ...
  const keys = {
    ...
    sosHistory: `relief_sos_history_${user.id}`,
    emergencyPref: `relief_emergency_pref_${user.id}`
  };
  ...
  const sosHistory = JSON.parse(localStorage.getItem(keys.sosHistory) || '[]');
  const emergencyPref = JSON.parse(localStorage.getItem(keys.emergencyPref) || '{"autoCall":true,"shareLocation":true}');
  ...
  function persist() {
    ...
    localStorage.setItem(keys.emergencyPref, JSON.stringify(emergencyPref));
  }
  ...
  function render() {
    ...
    document.getElementById('sos-history-list').innerHTML = sosHistory.length ? sosHistory.map((s)=>`<li>${new Date(s.date).toLocaleString()} - ${s.provider} (${s.status}) ETA ${s.eta}m</li>`).join('') : '<li>No SOS history yet.</li>';
    document.getElementById('pref-auto-call').checked = Boolean(emergencyPref.autoCall);
    document.getElementById('pref-share-location').checked = Boolean(emergencyPref.shareLocation);
  ...
  document.getElementById('save-emergency-pref').addEventListener('click', () => {
    emergencyPref.autoCall = document.getElementById('pref-auto-call').checked;
    emergencyPref.shareLocation = document.getElementById('pref-share-location').checked;
    persist();
    alert('Emergency preferences saved');
  });
